...
function renard_to_comment_type(&$content)
{
	preg_match('/<\$CommentType([^\$]+)\$>/', $content, $matches);
...
function renard_to_post_nav_link(&$content)
{
...
function renard_to_translatable(&$content, $tag)
{
	if ( $tag == '_e' )
	{
		$after = ' ?>';
...
function renard_to_from_element(&$content, $in_re, $tag)
{
...
function renard_parse_ez(&$content)
{
	renard_to_translatable($content, '__');
	...
	renard_to_from_element($content, '/<CurrentCategory>([^>]*)<\/CurrentCategory>/', 'single_cat_title');
	renard_to_from_element($content, '/<EditComment>([^>]*)<\/EditComment>/', 'edit_comment_link');
	renard_to_from_element($content, '/<EditEntry>([^<]*)<\/EditEntry>/', 'edit_post_link');
	renard_to_from_element($content, '/<EntryCategories>([^>]*)<\/EntryCategories>/', 'the_category');
	renard_to_from_element($content, '/<EntryContent>(.*)<\/EntryContent>/', 'the_content');

	renard_to_comment_type($content);
	renard_to_comments_number($content);
	renard_to_comments_rss_link($content);
	renard_to_post_nav_link($content);
	renard_to_translatable($content, '_e');

	renard_unwind($content);
}
